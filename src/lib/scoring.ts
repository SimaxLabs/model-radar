...
export function rankBudgetModels(models: RadarModel[]) {
  const eligible = models.filter(
    (model): model is RadarModel & { intelligence: number } =>
      model.isCheap && model.intelligence !== null && model.blendedPrice > 0,
  );
  ...
  const score = (model: RadarModel & { intelligence: number }) => {
